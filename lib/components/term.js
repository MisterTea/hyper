...
export default class Term extends Component {
  ...
  write(data) {
    ...
    // Watch for ansi escape codes for htm.
    for (var i=0;i<data.length;i++) {
      var c = data.charCodeAt(i);
      if ( this.escapeBuffer.length==0 && c == 0x1b) { // start of escape code
        this.escapeBuffer = data[i];
      } else if (this.escapeBuffer.length == 1 && c == 0x5b) { // [
        this.escapeBuffer += '['
      } else if (this.escapeBuffer.length<5 && c == 0x23) {
        this.escapeBuffer += '#';
      } else if (this.escapeBuffer.length==5 && c == 0x71) {
        // BEGIN HTM MODE!!!
        console.log("BEGINNING HTM MODE");
      } else {
        this.escapeBuffer = '';
      }
    }
    console.log("WRITING DATA: " + this.escapeBuffer.length);
